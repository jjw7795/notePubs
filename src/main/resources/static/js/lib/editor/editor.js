...
/*
 * var setting = { resourceUrl:null, targetSelector:null, uploadHandlerUrl:
 * null, contextName:null }; var editor = new Editor(setting); 또는 var editor =
 * new Editor(resourceUrl, targetSelector, uploadHandlerUrl);
 */
...
Editor.prototype = {
	...
	htmlLoadHandler:function(request){
		...
		var undoButton = this.toolbar.querySelector(".com.newlecture.single-editor>.toolbar .btn-undo");
		var redoButton = this.toolbar.querySelector(".com.newlecture.single-editor>.toolbar .btn-redo");
		...
		var boxButton = this.toolbar.querySelector(".com.newlecture.single-editor>.toolbar .btn-box");
		var adsenseButton = this.toolbar.querySelector(".com.newlecture.single-editor>.toolbar .btn-adsense");
		var tableButton = this.toolbar.querySelector(".com.newlecture.single-editor>.toolbar .btn-table");
		...
		// 이벤트 바인딩
		this.htmlArea.addEventListener("input", this.htmlAreaInputHandler.bind(this));
		...
		//undoButton.onclick = this.undoHandler.bind(this);
		//redoButton.onclick = this.redoHandler.bind(this);
		...
		addLinkButton.onclick = this.addLinkButtonClickHandler.bind(this);
		
		htmlButton.onclick = this.htmlButtonClickHandler.bind(this);
		codeButton.onclick = this.codeButtonClickHandler.bind(this);
		boxButton.onclick = this.boxButtonClickHandler.bind(this);
		adsenseButton.onclick = this.adsenseButtonClickHandler.bind(this);
		tableButton.onclick = this.tableButtonClickHandler.bind(this);
		
		// <textarea>에 있던 텍스트를 편집기로 옮긴다.
		this.htmlArea.innerHTML = this.textArea.value;
		this.htmlArea.ondblclick = this.htmlAreaDblClickHandler.bind(this);
		

	},
	
	htmlAreaInputHandler:function(e){
		e.preventDefault();
		/*=== 에디터의 입력이 변경되면 그것을 TextArea로 이전 시킴 =======================================================*/
		
		this.textArea.value = this.htmlArea.innerHTML;		
	},
	htmlAreaDblClickHandler:function(e){
		// 테이블의 tr추가 버튼이 클릭되면 실행되는 코드
		if(e.target.classList.contains("btn-add-tr")){
			var thead = e.target.parentNode.parentNode;
			var tbody = thead.nextElementSibling;
			var newTr = document.createElement("tr");
			var tdsHTML = "";
			
			var headTr = thead.children[0];
			var headTdCount = headTr.children.length;
			for(var i=0; i<headTdCount; i++)
				tdsHTML += "<td></td>";
			
			newTr.innerHTML = tdsHTML;
		    tbody.appendChild(newTr);
		}
		else if(e.target.classList.contains("btn-remove-tr")){
			var tbody = e.target.parentNode.parentNode.nextElementSibling;
	...
	undoHandler:function(e){
		...
		//this.htmlEditor.undoManager.undo();
	},
	...
	addLinkButtonClickHandler:function(e){
		alert("aa");
	},
	...
	codeButtonClickHandler:function(e){
		...
		html = '<div style="border:2px solid #797979;padding:10px;background:#000;"> \
					<pre style="min-height:20px;background:#000;color:#fff;"><code>//여기에 코드를 작성해주세요.</code></pre> \
					<pre style="min-height:20px;background:#000;color:#fff;"><code></code></pre> \
				</div> \
				<div> \
					&nbsp; \
				</div>';
		...
		/*new Modal("input:textarea:code")
		.addEventListener("ok", this.codeEditOkayHandler.bind(this))
        .show();*/
	},
	boxButtonClickHandler:function(e){
		var sel, range, html;
		
		html = '<div style="border:1px solid #797979;padding:10px;">&nbsp;</div><div>&nbsp;</div>';
		
		this.pasteHtmlAtCaret(html);
	},
	adsenseButtonClickHandler:function(e){
		var sel, range, html;
		
		html = '<div style="border:dotted 1px #797979;min-height:20px;" contentEditable="false"> \
				구글 광고가 추가되었습니다. \
				...
				<ins class="adsbygoogle" \
				     ...
				     data-ad-client="ca-pub-1880024449642113" \
				     data-ad-slot="5814261785"></ins> \
				<script> \
	...
	tableButtonClickHandler:function(e){
		var sel, range, html;
		
		html = '<table> \
					<thead><tr><td class="btn-add-tr"></td><td class="btn-remove-tr"></td></tr></thead> \
					<tbody> \
						<tr><td></td><td></td></tr> \
					</tbody> \
				</table>';
	...
	codeEditOkayHandler:function(data){		
		
		var sel, range, html;
		
		html = '<div contenteditable="false" ><pre style="min-height:20px;border:1px solid #e9e9e9;"><code>'+data+'</code></pre></div><div style="text-align:center;">코드명:</div>';
	    ...
	    */
		...
		//document.execCommand("insertHTML", true, '<div contenteditable="false" ><pre style="min-height:20px;border:1px solid #e9e9e9;"><code>'+data+'</code></pre></div><div style="text-align:center;">코드명:</div>');
...
/*		var img = document.createElement('img');
	    ...
	    if (nodeContainsSelection(this.htmlArea)) {

	    	console.log("선택된 노드가 있다야..");
	...
	getHtml:function(){
		var html = this.htmlArea.innerHTML;
		return html;
	}
};
